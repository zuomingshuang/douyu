...
def start(roomid):
    ...
    while True:
        ...
        if not data:
            ...
            with open('douyu_danmu_%s.txt'%get_name(roomid), 'a',encoding='utf-8') as f:
                try:
                    for i in danmu_more:

                        user_name= i[0].decode(encoding='utf-8', errors='ignore')
                        content= i[1].decode(encoding='utf-8', errors='ignore')

                        print('昵称:%s   弹幕内容:%s'%(user_name,content))
                        f.write('昵称:%s   弹幕内容:%s\n'%(user_name,content))
                        danmuNum = danmuNum + 1
                except:
                    continue
...
def get_name(roomid):
    ...
    return soup.find('head').find('title').text
